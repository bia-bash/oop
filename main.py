...
def format_time_12hr(time_str):
    """Convert 'HH:MM' 24-hour string to 'hh:mm AM/PM', or return 'N/A' if invalid."""
    ...
    if st.button("Add Patient"):
        if not patient_id:
            ...
            st.error("Time must be in 12-hour format like 02:30 PM.")
                 ...
                 p.get("assigned_doctor") == assigned_doctor and
...
""")
        if st.button("Print All Patients"):
            st.write("\n".join([f"{i+1}. {p.get('name', 'N/A')} ({p.get('appointment_date', 'N/A')}) - {p.get('assigned_doctor', 'N/A')}" for i, p in enumerate(filtered_patients)]))
    else:
...
# --- Follow-Up Patients ---
    ...
    doctor_filter = st.selectbox("Filter by Doctor", ["All"] + sorted(set(p.get('assigned_doctor', '') for p in follow_ups)))
...
# --- Patient Schedule ---
    ...
    if not active_patients:
        ...
        if filtered:
            ...
            def sort_key(p):
                date_str = p.get("appointment_date", "1900-01-01")
                time_str = p.get("appointment_time", "00:00")
                try:
                    return dt.strptime(date_str + " " + time_str, "%Y-%m-%d %H:%M")
                except ValueError:
                    return dt.min
